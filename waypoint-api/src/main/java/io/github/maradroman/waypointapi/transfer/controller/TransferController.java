...
public class TransferController {
    ...
    @Operation(
            ...
            description = "Returns all transfers for a goal, ordered by most recent first")
    @ApiResponses({
    ...
    @Operation(
            ...
            description =
                    "Allocate funds from the goal wallet to a milestone. Cannot exceed wallet balance or milestone remaining need.")
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "Funds allocated"),
        @ApiResponse(
                responseCode = "400",
                description = "Allocation failed (insufficient funds or milestone fully funded)"),
        @ApiResponse(responseCode = "401", description = "Unauthorized"),
    ...
    public ResponseEntity<ResponseEnvelope<AllocateResponse>> allocate(
            @CurrentUser User user, @PathVariable UUID goalId, @Valid @RequestBody AllocateRequest request) {
    ...
    @Operation(
            ...
            description = "Withdraw previously allocated funds from a milestone back to the goal wallet")
    @ApiResponses({
    ...
    public ResponseEntity<ResponseEnvelope<AllocateResponse>> withdraw(
            @CurrentUser User user, @PathVariable UUID goalId, @Valid @RequestBody WithdrawRequest request) {
    ...
    public ResponseEntity<ResponseEnvelope<TransferResponse>> updateTransfer(
        ...
        return ResponseEntity.ok(ResponseEnvelope.of(transferService.updateTransfer(user, goalId, id, request)));
    }
