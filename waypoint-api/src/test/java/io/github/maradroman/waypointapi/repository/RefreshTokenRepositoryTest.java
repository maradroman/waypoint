...
import io.github.maradroman.waypointapi.testdata.TestDataRefreshTokenEntity;
...
class RefreshTokenRepositoryTest extends TestDataJpa {
    ...
    void findByToken_returnsToken_whenExistsTest() {
        ...
        jdbcTemplate.update("""
                ...
                VALUES (?, ?, ?, ?, ?)
                """, rtId, user.getId(), REFRESH_TOKEN_VALUE, java.time.Instant.now().plusSeconds(2592000), java.time.Instant.now());
        flushAndClear();
